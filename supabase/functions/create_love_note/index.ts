...
serve(async (req) => {
  ...
  const coupleId = String(body?.couple_id ?? '').trim();
  if (!coupleId || !body?.text) return new Response(JSON.stringify({ error: 'Missing required fields' }), { status: 400, headers: { ...headers, 'content-type': 'application/json' } });
  ...
  const { data: loveNote, error: loveNoteErr } = await admin.from('love_notes').insert({
    couple_id: coupleId,
    from_id: uData.user.id,
    to_id: body?.to_id ?? null,
    emotion: body?.emotion ?? null,
    topic: body?.topic ?? null,
    text: body?.text,
  }).select('*').single();
  if (loveNoteErr || !loveNote) return new Response(JSON.stringify({ error: loveNoteErr?.message ?? 'Failed to create love note' }), { status: 400, headers: { ...headers, 'content-type': 'application/json' } });
  return new Response(JSON.stringify({ love_note: loveNote }), { status: 200, headers: { ...headers, 'content-type': 'application/json' } });
});
