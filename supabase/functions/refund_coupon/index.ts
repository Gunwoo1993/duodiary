...
serve(async (req) => {
  ...
  if ((coupon as any).sender_id !== userId) {
    return new Response(JSON.stringify({ error: 'Only the sender can refund this coupon' }), {
      status: 403,
      headers: { ...headers, 'content-type': 'application/json' }
    });
  }
  ...
  if (updateErr || !updated) {
    return new Response(JSON.stringify({ error: updateErr?.message ?? 'Failed to refund coupon' }), {
      status: 400,
      headers: { ...headers, 'content-type': 'application/json' }
    });
  }
