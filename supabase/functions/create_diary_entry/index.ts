...
serve(async (req) => {
  ...
  const { data: entry, error: entryErr } = await admin.from('diary_entries').insert({
    couple_id: coupleId,
    author_id: uData.user.id,
  ...
  if (entryErr || !entry) return new Response(JSON.stringify({ error: entryErr?.message ?? 'Failed to create diary entry' }), { status: 400, headers: { ...headers, 'content-type': 'application/json' } });
  return new Response(JSON.stringify({ entry }), { status: 200, headers: { ...headers, 'content-type': 'application/json' } });
});
