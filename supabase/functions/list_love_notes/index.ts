...
serve(async (req) => {
  ...
  const body = await req.json().catch(() => null) as { couple_id?: string; limit?: number } | null;
  ...
  let query = admin.from('love_notes').select('*').eq('couple_id', coupleId).order('created_at', { ascending: false });
  if (body?.limit) query = query.limit(body.limit);
  const { data: loveNotes, error: loveNotesErr } = await query;
  if (loveNotesErr) return new Response(JSON.stringify({ error: loveNotesErr.message }), { status: 400, headers: { ...headers, 'content-type': 'application/json' } });
  return new Response(JSON.stringify({ love_notes: loveNotes ?? [] }), { status: 200, headers: { ...headers, 'content-type': 'application/json' } });
});
