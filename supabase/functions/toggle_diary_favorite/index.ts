...
serve(async (req) => {
  ...
  const body = await req.json().catch(() => null) as { id?: string; is_favorite?: boolean } | null;
  ...
  const { data: memberships, error: membershipErr } = await admin.from('couple_members').select('user_id').eq('couple_id', (entry as any).couple_id);
  ...
  const { error: updateErr } = await admin.from('diary_entries').update({ is_favorite: Boolean(body?.is_favorite) }).eq('id', entryId);
  if (updateErr) return new Response(JSON.stringify({ error: updateErr.message }), { status: 400, headers: { ...headers, 'content-type': 'application/json' } });
