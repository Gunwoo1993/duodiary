...
serve(async (req) => {
  ...
  const body = await req.json().catch(() => null) as { couple_id?: string; title?: string; date?: string } | null;
  const coupleId = body?.couple_id?.trim();
  if (!coupleId || !body?.title || !body?.date) return new Response(JSON.stringify({ error: 'Missing required fields' }), { status: 400, headers: { ...headers, 'content-type': 'application/json' } });
  ...
  const { data: anniversary, error: anniversaryErr } = await admin.from('anniversaries').insert({
    couple_id: coupleId,
    title: body.title,
    date: body.date,
  }).select('*').single();
  if (anniversaryErr || !anniversary) return new Response(JSON.stringify({ error: anniversaryErr?.message ?? 'Failed to create anniversary' }), { status: 400, headers: { ...headers, 'content-type': 'application/json' } });
  return new Response(JSON.stringify({ anniversary }), { status: 200, headers: { ...headers, 'content-type': 'application/json' } });
});
