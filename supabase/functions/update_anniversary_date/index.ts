...
serve(async (req) => {
  ...
  const body = await req.json().catch(() => null) as { couple_id?: string; anniversary_date?: string | null } | null;
  ...
  const { data: couple, error: coupleErr } = await admin.from('couples').update({ anniversary_date: body?.anniversary_date ?? null }).eq('id', coupleId).select('*').single();
  if (coupleErr || !couple) return new Response(JSON.stringify({ error: coupleErr?.message ?? 'Failed to update anniversary' }), { status: 400, headers: { ...headers, 'content-type': 'application/json' } });
  return new Response(JSON.stringify({ couple }), { status: 200, headers: { ...headers, 'content-type': 'application/json' } });
});
